...
- (nonnull UITableViewCell *)tableView:(nonnull UITableView *)tableView cellForRowAtIndexPath:(nonnull NSIndexPath *)indexPath {
    ...
    switch (cellType) {
        ...
        case TopAddDeviceTypeVCCellTypeDBTV:
            cell.textLabel.text = NSLocalizedString(@"TV", @"");
            break;
        case TopAddDeviceTypeVCCellTypeDBSTB:
            cell.textLabel.text = NSLocalizedString(@"STB", @"");
            break;
        case TopAddDeviceTypeVCCellTypeDBIPTV:
            cell.textLabel.text = NSLocalizedString(@"IPTV", @"");
            break;
        case TopAddDeviceTypeVCCellTypeDBAC:
            cell.textLabel.text = NSLocalizedString(@"AC", @"");
            break;
        case TopAddDeviceTypeVCCellTypeCustom:
            cell.textLabel.text = NSLocalizedString(@"Custom", @"");
            break;
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSNumber * cellTypeNum = self.cellTypeList[indexPath.section];
    TopAddDeviceTypeVCCellType cellType =   cellTypeNum.integerValue;
    GLDatabaseDevType dataBaseDeviceType = GLDatabaseDevTypeAC;
    switch (cellType) {
    
        case TopAddDeviceTypeVCCellTypeDBTV:
            dataBaseDeviceType = GLDatabaseDevTypeTV;
            break;
        case TopAddDeviceTypeVCCellTypeDBSTB:
             dataBaseDeviceType = GLDatabaseDevTypeSTB;
            break;
        case TopAddDeviceTypeVCCellTypeDBIPTV:
             dataBaseDeviceType = GLDatabaseDevTypeIPTV;
            break;
        case TopAddDeviceTypeVCCellTypeDBAC:
             dataBaseDeviceType = GLDatabaseDevTypeAC;
            break;
        case TopAddDeviceTypeVCCellTypeCustom: {
            TopSubDevInfo * subDevice = [[TopSubDevInfo alloc] init];
            subDevice.mainType = GLDeviceMainTypeCustomDev;
            subDevice.subId = 0;
            subDevice.databaseDevType = 0;
            subDevice.carrierType = GLCarrierTypeCARRIER38;
            subDevice.fileId = 0;
            [[TopGLSmartPiAPIManager shareManager] setSubDeviceWithMd5:self.mainDeviceInfo.md5 subDevInfo:subDevice action:GLActionFullTypeInsert complete:^(TopGLSmartPiResultInfo * _Nonnull resucltInfo) {

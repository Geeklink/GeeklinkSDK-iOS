...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [TopGLSmartPiAPIManager.shareManager controlSubDeviceWithMd5:self.mainDeviceInfo.md5 andIrCode:self.codeList[indexPath.row] complete:^(TopGLSmartPiResultInfo * _Nonnull resucltInfo) {
...
- (IBAction)clickedAddBtn:(id)sender {
    ...
    [alertVC addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"Cancel", nil) style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
        [TopGLSmartPiAPIManager.shareManager getCodeFromDeviceWithMd5:self.mainDeviceInfo.md5 andCodeType:GLKeyStudyTypeKeyStudyCancel complete:^(TopGLSmartPiResultInfo * _Nonnull resucltInfo) {
            if (resucltInfo.state == GLStateTypeOk) {
    ...
    [TopGLSmartPiAPIManager.shareManager getCodeFromDeviceWithMd5:self.mainDeviceInfo.md5 andCodeType:GLKeyStudyTypeKeyStudyIr complete:^(TopGLSmartPiResultInfo * _Nonnull resucltInfo) {
        if (resucltInfo.state == GLStateTypeOk) {

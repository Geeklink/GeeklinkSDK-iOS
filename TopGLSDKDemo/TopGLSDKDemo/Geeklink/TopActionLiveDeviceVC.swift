...
import UIKit
enum TopActionType: Int {
    case macroAction
    case miniPiSubDeviceAction
}
class TopActionLiveDeviceVC: TopSuperVC, TopCodeViewDelegate,   TopSpecialActionViewDelegate,TVSTBNumberViewDelegate,     TopACCodeViewDelegate {
    ...
    func initSubViews() -> Void {
        if deviceInfo.mainType ==  .database{
            ...
            case .AC:
                ...
                self.navigationItem.rightBarButtonItem = UIBarButtonItem.init(barButtonSystemItem: .done, target: self, action: #selector(onClickSaveButton))
            case .TV:
            ...
            case .STB:
                ...
                let viewSTB = TopSTBCodeView()
                viewSTB.delegate = self
                self.stbCodeView = viewSTB
                viewSTB.hideChannelBtn(true)
                self.view.addSubview(viewSTB)
    ...
    func specialActionViewDidClickedBtn(_ index: Int32) {
        ...
        if deviceInfo.mainType == .geeklink{
            ...
            case .plug, .plugPower, .plugFour:
                if index == 0 {
                    self.setMacroAction(value: "11")
                }else {
                    self.setMacroAction(value: "10")
                }
    ...
    func iPTVCodeViewDelegate(_ keyType: GLKeyType, databaceType: GLDbIptvKeyType, view: UIView) {
     ...
     func numberViewDidclickedBtn(_ index: Int, btn: UIButton) {
        ...
        if deviceInfo.mainType == .database {
            self.addDatabaseNumberKey(index)
        }else {
    ...
    func addDatabaseNumberKey(_ index: Int) -> Void {
        let databaseType = GLDatabaseType(rawValue: Int(deviceInfo.subType))!
        if databaseType == .TV {
